...
def carregar_temporal():
    ...
    # Preenche DataFrame com segurança
    if resultado:
        df_av = pd.DataFrame([{
            'ano': r['_id']['ano'],
            'mes': r['_id']['mes'],
        ...
        df_av['mes_ano'] = pd.to_datetime(
            df_av['ano'].astype(str) + '-' + df_av['mes'].astype(str).str.zfill(2),
            format='%Y-%m'
        )
    else:
        df_av = pd.DataFrame(columns=['ano', 'mes', 'media_nota', 'qtde_avaliacoes', 'mes_ano'])

    return df_mysql, df_obj, df_av



df_mysql, df_obj, df_av = carregar_temporal()

# Se você precisar usar algum dos DataFrames individualmente, faça assim:
df_vendas = df_mysql  # Por exemplo, se quiser trabalhar com o DataFrame do MySQL
...
def carregar_temporal():
    ...
    # Cria a coluna 'mes_ano' corretamente a partir do ano e mês
    df_av['mes_ano'] = pd.to_datetime(
    df_av['ano'].astype(str) + '-' + df_av['mes'].astype(str).str.zfill(2),
    format='%Y-%m'
)
...
def carregar_clustering():
    ...
    df = df.fillna(0)  # substitui todos os NaNs por 0
