...
with tab1:
    ...
    roll_up = df.groupby(['nome_categoria', 'ano']) \
                 .agg({'valor_total': 'sum'}) \
                 .reset_index()
    roll_up.rename(columns={'nome_categoria': 'Categoria', 'ano': 'Ano', 'valor_total': 'Valor Total'}, inplace=True)
...
with tab2:
    ...
    drill_down = df.groupby(['nome_categoria', 'ano', 'mes']) \
                   .agg({'valor_total': 'sum'}) \
                   .reset_index()
    drill_down.rename(columns={'nome_categoria': 'Categoria', 'ano': 'Ano', 'mes': 'Mês', 'valor_total': 'Valor Total'}, inplace=True)
    st.dataframe(drill_down)
    
    categoria_selecionada = st.selectbox("Selecione uma categoria para visualização detalhada:", df['nome_categoria'].unique())
...
if len(df_filtrado_forecast) < 12:
    ...
    try:
        model = SARIMAX(train['valor_vendido'], order=(1,1,1), seasonal_order=(1,1,1,12), enforce_stationarity=False, enforce_invertibility=False)
        results = model.fit(disp=False)
        forecast = results.get_forecast(steps=len(test) + 6)
        ...
        future_df = pd.DataFrame({'Mês': future_dates, 'Previsão (R$)': future_predictions.values})
...
# ============================
st.subheader(" dbc Quantidade Total em Estoque por Loja")
